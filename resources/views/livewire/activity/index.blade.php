  ...
  <div class="w-full overflow-x-auto">
    ...
    @else
      <table class="min-w-full">
        <thead>
          <tr class="max-w-max border-y border-gray-100 dark:border-gray-800">
            <th class="py-3">
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          @foreach ($activities as $activity)
            <tr wire:key="{{ $activity->slug }}">
              <td class="py-3">
                <div class="flex max-w-max items-center gap-3">
              ...
              <td class="py-3">
                <div class="">
                  <p class="text-theme-sm text-gray-500 dark:text-gray-400">
                    {{ Helper::getPhotoCount($activity) }} Foto
                  </p>
                  <p class="text-theme-sm text-gray-500 dark:text-gray-400">
                    {{ Helper::getVideoCount($activity) }} Video
                  </p>
                </div>
              </td>
              <td class="py-3">
              ...
              <td class="flex gap-1.5 py-3">
                <div class="group relative inline-block">
                  <a class="bg-brand-500 hover:bg-brand-600 shadow-theme-xs inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 px-4 py-2.5 text-white dark:border-gray-700"
                    href="{{ route('activity.drive', ['activity' => $activity]) }}">
                  ...
                  <span
                    class="pointer-events-none absolute left-0 top-1/2 mr-2 max-w-max -translate-x-full -translate-y-1/2 rounded-md bg-gray-700 px-3 py-1 text-sm text-white opacity-0 shadow-lg transition-opacity duration-200 group-hover:opacity-100">
                    Drive
                  </span>
                </div>
                <div class="group relative inline-block">
                ...
                @if ($activity->published)
                  <div class="group relative inline-block">
                    <button
                      class="bg-warning-500 hover:bg-warning-600 shadow-theme-xs inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 px-4 py-2.5 text-white dark:border-gray-700"
                      type="button" @click="$dispatch('unpublish-activity', { activity: '{{ $activity->slug }}'} )">
                      <svg class="size-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        stroke-width="1.5" stroke="currentColor">
                ...
                @else
                  <div class="group relative inline-block">
                    <button
                      ...
                      type="button" @click="$dispatch('publish-activity', { activity: '{{ $activity->slug }}'} )">
                      <svg class="size-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        stroke-width="1.5" stroke="currentColor">
                ...
                @endif
                <div class="group relative inline-block">
                  <a class="bg-warning-500 hover:bg-warning-600 shadow-theme-xs inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 px-4 py-2.5 text-white dark:border-gray-700"
                    href="{{ route('activity.edit', ['activity' => $activity]) }}">
                    <svg class="size-5" xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                      viewBox="0 0 24 24">
                      <path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                        stroke-width="2" d="M4 20h4L18.5 9.5a2.828 2.828 0 1 0-4-4L4 16zm9.5-13.5l4 4" />
                    </svg>
                  </a>
                  <span
                    class="pointer-events-none absolute left-0 top-1/2 mr-2 max-w-max -translate-x-full -translate-y-1/2 rounded-md bg-gray-700 px-3 py-1 text-sm text-white opacity-0 shadow-lg transition-opacity duration-200 group-hover:opacity-100">
      ...
      @endif
      <livewire:modal.activity.publish />
      <livewire:modal.activity.unpublish />
      <livewire:modal.activity.delete />
    @endif
  </div>
</div>
