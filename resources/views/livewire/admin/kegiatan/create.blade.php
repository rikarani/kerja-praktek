<form class="mt-4 space-y-4">
  <div class="space-y-4 md:flex md:gap-4 md:space-y-0">
    <div class="flex-1">
      <label for="title" wire:loading.class="opacity-50" wire:target="save, saveAndPublish" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
        Judul Kegiatan <span class="text-red-500">*</span>
      </label>
      <div class="relative">
        <input required id="title" type="text" wire:model="title" wire:loading.attr="disabled" wire:target="save, saveAndPublish" placeholder="makan siang gratis"
    ...
    <div class="flex-1">
      <label wire:loading.class="opacity-50" wire:target="save, saveAndPublish" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
        Bentuk Kegiatan <span class="text-red-500">*</span>
      </label>
      <div class="relative z-20 bg-transparent">
        <select wire:loading.attr="disabled" wire:target="save, saveAndPublish" required wire:model="type"
          ...
          <option disabled selected value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
            Pilih Opsi
          </option>
          <option value="seminar" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
            Seminar
          </option>
          <option value="lomba" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
            Lomba / Kompetisi
          </option>
          <option value="upacara" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">
            Upacara
          </option>
        </select>
        @error('type')
    ...
    <div class="flex-1">
      <label wire:loading.class="opacity-50" class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
        Tanggal Kegiatan <span class="text-red-500">*</span>
      </label>
      <div wire:ignore class="relative">
        <input required wire:model="start_date" type="date" placeholder="Pilih Tanggal"
  ...
  <div>
    ...
    <textarea required wire:loading.attr="disabled" wire:target="save, saveAndPublish" wire:model="description" placeholder="kegiatan ini blablabla" rows="6"
      class="{{ twMerge('disabled:opacity-50 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 focus:ring-3 focus:outline-hidden w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 dark:bg-gray-900 dark:border-gray-700 dark:text-white/90 dark:placeholder:text-white/30', $errors->has('description') ? 'border-error-300 focus:border-error-300 focus:ring-error-500/10 dark:border-error-700 dark:focus:border-error-800' : '') }}"></textarea>
    @error('description')
  ...
  <div>
    ...
    <x-filepond::upload required wire:model="documentations" multiple allow-reorder allow-file-type-validation accepted-file-types="image/png,image/jpeg,image/jpg,video/mp4" allow-file-size-validation max-file-size="10MB" />
  ...
  <div class="space-y-4 md:flex md:justify-end md:gap-4 md:space-y-0">
    <div class="w-full md:w-auto">
      <button type="button" wire:click="save" wire:loading.attr="disabled" wire:target="save, saveAndPublish"
        ...
        <div wire:loading wire:target="save" role="status">
          <svg aria-hidden="true" class="inline size-5 animate-spin fill-gray-600 text-gray-200 dark:fill-gray-300 dark:text-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
    ...
    <div class="w-full md:w-auto">
      <button type="button" wire:click="saveAndPublish" wire:loading.attr="disabled" wire:target="save, saveAndPublish"
        class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 flex items-center justify-center gap-2 rounded-lg px-5 py-3.5 text-sm font-medium text-white disabled:opacity-50">
        <div wire:loading wire:target="saveAndPublish" role="status">
          <svg aria-hidden="true" class="inline size-5 animate-spin fill-gray-600 text-gray-200 dark:fill-gray-300 dark:text-gray-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
              fill="currentColor" />
