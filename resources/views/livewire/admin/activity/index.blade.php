    ...
    <div class="flex items-center gap-3">
      ...
      <button
        ...
        <svg class="fill-white stroke-current dark:fill-gray-800" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M7.91745 11.525C6.49762 11.525 5.34662 12.676 5.34662 14.0959C5.34661 15.5157 6.49762 16.6667 7.91745 16.6667C9.33728 16.6667 10.4883 15.5157 10.4883 14.0959C10.4883 12.676 9.33728 11.525 7.91745 11.525Z" fill=""
            stroke="" stroke-width="1.5" />
        </svg>
        Filter
      </button>
      <a href="{{ route('kegiatan.create') }}" class="bg-brand-500 text-theme-sm hover:bg-brand-600 shadow-theme-xs inline-flex items-center gap-2 rounded-lg border border-gray-300 px-4 py-2.5 font-medium text-white dark:border-gray-700">
            ...
            <tr>
              ...
              <td class="py-3">
                <div class="flex items-center">
                  ...
                  @else
                    <span class="bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500 inline-flex items-center justify-center gap-1 rounded-full px-2.5 py-0.5 text-sm font-medium">
                      Belum Publikasi
                    </span>
                  @endif
                </div>
              ...
              <td class="py-3">
                <div class="flex items-center">
                  <a target="_blank" href="{{ Storage::disk('google')->url($activity->title) }}" class="text-theme-sm text-gray-500 dark:text-gray-400">
                    {{ Str::limit(Storage::disk('google')->url($activity->title), 45) }}
                  </a>
                </div>
              </td>
              <td class="space-y-2 py-3">
                <div>
                  @if ($activity->published)
                    <button wire:click="unpublish('{{ $activity->id }}')"
                  ...
                  @else
                    <button wire:click="publish('{{ $activity->id }}')"
                ...
                <div class="flex gap-2">
                  <button class="bg-blue-light-500 hover:bg-blue-light-600 shadow-theme-xs inline-flex flex-1 items-center justify-center gap-2 rounded-lg border border-gray-300 px-4 py-2.5 text-white dark:border-gray-700">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                  ...
                  <button type="button" wire:click="hapus('{{ $activity->id }}')" wire:confirm="Apakah anda yakin ingin menghapus data ini?"
