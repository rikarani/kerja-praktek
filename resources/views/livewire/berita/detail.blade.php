          ...
          @if ($videos->isNotEmpty())
            <div class="grid gap-4 sm:grid-cols-3">
              @foreach ($videos as $video)
                @php
                  $id = Helper::extractID($video);
                  $thumb = "https://drive.google.com/thumbnail?id=$id";
                @endphp
                <div class="aspect-video overflow-hidden rounded-lg">
                  <a class="mt-3 block aspect-square w-full overflow-hidden rounded-lg bg-gray-100"
                     href="{{ $video }}" target="_blank">
                    <img class="h-full w-full object-cover" src="{{ $thumb }}" alt="{{ $video }}" />
                  </a>
                </div>
              @endforeach
            </div>
