      ...
      @else
        <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          @foreach ($activities as $activity)
            @php
              $files = Storage::disk('google')->allFiles("$activity->year/$activity->title/dokumentasi");
              $firstImage = Collection::make($files)->filter(fn ($file) => in_array(pathinfo($file, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png']))->first();
